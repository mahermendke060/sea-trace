...
export const DistributorLocationDialog = ({ open, onOpenChange, location, onSuccess }: LocationDialogProps) => {
  ...
  useEffect(() => {
    if (location) {
      ...
      setScope("internal");
      setFormData({ name: "", type: "wharf", address: "" });
    }
  }, [location, open]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload = { ...formData, address: scope };

    if (location) {
      const { error } = await supabase.from("distributor_locations").update(payload).eq("id", location.id);
      if (error) {
      ...
      const { data, error } = await supabase.from("distributor_locations").insert([payload]).select().single();
      if (error) {
        toast({ title: "Error", description: error.message, variant: "destructive" });
        return;
      }
      ...
      <DialogContent>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="name" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} required placeholder="e.g., Harbor Bay Wharf" />
          </div>
          <div className="space-y-2">
          ...
          <div className="space-y-2">
            ...
            <Textarea id="address" value={formData.address} onChange={(e) => setFormData({ ...formData, address: e.target.value })} rows={3} placeholder="Enter full address" />
          </div>
