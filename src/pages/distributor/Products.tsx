...
export default function DistributorProducts() {
  ...
  const fetchProducts = async () => {
    const { data } = await supabase.from("products").select("*").order("species");
    setProducts(data || []);
  };
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage product species</p>
        </div>
        <Button onClick={() => { setSelectedProduct(null); setDialogOpen(true); }} className="gap-2 bg-emerald-600 hover:bg-emerald-700">
